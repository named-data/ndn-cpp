   ...
   const EncryptError::OnError& onError)
{
  ...
  contentKeyName.append(toIsoString(hourSlot));
  ...
  Exclude timeRange;
  excludeAfter(timeRange, Name::Component(toIsoString(timeSlot)));
  for (map<Name, ptr_lib::shared_ptr<KeyInfo> >::iterator i = eKeyInfo_.begin();
    ...
    if (timeSlot < keyInfo.beginTimeSlot || timeSlot >= keyInfo.endTimeSlot) {
      ...
      eKeyName.append(toIsoString(keyInfo.beginTimeSlot));
      eKeyName.append(toIsoString(keyInfo.endTimeSlot));
      encryptContentKey
   ...
   const EncryptError::OnError& onError)
{
  ...
  dataName.append(toIsoString(timeSlot));
   ...
   const EncryptError::OnError& onError)
{
  ...
  MillisecondsSince1970 begin = fromIsoString
    (keyName.get(START_TIME_STAMP_INDEX).getValue().toRawStr());
  MillisecondsSince1970 end = fromIsoString
   ...
   const EncryptError::OnError& onError)
{
  ...
  keyName.append(toIsoString(getRoundedTimeSlot(timeSlot)));
