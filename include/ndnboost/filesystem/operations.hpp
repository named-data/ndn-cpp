...
namespace ndnboost
{
  namespace filesystem
  {
    ...
    class NDNBOOST_SYMBOL_VISIBLE filesystem_error : public system::system_error
    {
      ...
      // functions previously throw() are now NDNBOOST_NOEXCEPT_OR_NOTHROW
      // functions previously without throw() are now NDNBOOST_NOEXCEPT

    public:
      // compiler generates copy constructor and copy assignment

      filesystem_error(
        const std::string & what_arg, system::error_code ec) NDNBOOST_NOEXCEPT
      ...
      filesystem_error(
        ...
        system::error_code ec) NDNBOOST_NOEXCEPT
        : system::system_error(ec, what_arg)
      {
        try
        {
      ...
      filesystem_error(
        const std::string & what_arg, const path& path1_arg,
        const path& path2_arg, system::error_code ec) NDNBOOST_NOEXCEPT
        : system::system_error(ec, what_arg)
      {
        try
        {
      ...
      ~filesystem_error() NDNBOOST_NOEXCEPT_OR_NOTHROW{}

        const path& path1() const NDNBOOST_NOEXCEPT
      {
        static const path empty_path;
        return m_imp_ptr.get() ? m_imp_ptr->m_path1 : empty_path;
      }
        const path& path2() const  NDNBOOST_NOEXCEPT
      {
        static const path empty_path;
        return m_imp_ptr.get() ? m_imp_ptr->m_path2 : empty_path;
      }

        const char* what() const NDNBOOST_NOEXCEPT_OR_NOTHROW
      {
        if (!m_imp_ptr.get())
...
//--------------------------------------------------------------------------------------//
  ...
  //  namespace ndnboost template specializations
  template<>
  struct range_mutable_iterator<ndnboost::filesystem::directory_iterator>
    { typedef ndnboost::filesystem::directory_iterator type; };
  template<>
  struct range_const_iterator <ndnboost::filesystem::directory_iterator>
    { typedef ndnboost::filesystem::directory_iterator type; };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    struct recur_dir_itr_imp
    {
      ...
      bool push_directory(system::error_code& ec) NDNBOOST_NOEXCEPT;
    ...
    inline
    bool recur_dir_itr_imp::push_directory(system::error_code& ec) NDNBOOST_NOEXCEPT
    ...
    // regardless of errors.
    {
      ...
      if (ec_push_directory)
      {
        ...
        {
          NDNBOOST_FILESYSTEM_THROW(filesystem_error(
...
#   endif
  ...
  //  namespace ndnboost template specializations
  template<>
  struct range_mutable_iterator<ndnboost::filesystem::recursive_directory_iterator>
                        { typedef ndnboost::filesystem::recursive_directory_iterator type; };
  template<>
  struct range_const_iterator <ndnboost::filesystem::recursive_directory_iterator>
                        { typedef ndnboost::filesystem::recursive_directory_iterator type; };
