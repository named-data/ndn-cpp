...
public:
   ...
   * ndn_AES_BLOCK_SIZE. This value is proved as a safety check that the
   ...
   * this is not ndn_AES_BLOCK_SIZE. This value is proved as a safety check
   ...
   * size is not ndn_AES_BLOCK_SIZE. This value is proved as a safety check
   * that the correct algorithm is being used.
   ...
   * must provide a large enough buffer, which should be at least
   * encryptedDataLength + ndn_AES_BLOCK_SIZE bytes.
